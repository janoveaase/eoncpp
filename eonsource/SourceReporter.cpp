...
namespace eon::source
{
	...
	Message::SourceIndicator::SourceIndicator( const Ref& source )
	{
		Source = source;
	...
	bool Message::SourceIndicator::_write( string& str )
	{
		...
		if( Start.Line == Source.end().Line )
			return false;
		Start = Source.source().getPosAtOffset( Line.end(), 1 );
		return true;
	}
	...
	{
		string str = "\n";
		Line = Ref( Source.source(), Start, Start );
	...
	size_t Message::SourceIndicator::_selectLineLength() const
	{
	...
	void Reporter::report( severity level, string&& text, const Ref& source )
	{
		...
		{
			if( level >= target.target().filterHigh() && level <= target.target().filterLow() )
				target.target().report( message );
		}
	}
}
