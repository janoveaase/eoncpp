...
namespace eon
{
	TEST( Source, basic )
	{
		...
		WANT_EQ( "This is a 2 line test", src.line( 0 ).stdstr() )
			<< "Wrong first line!";
	...
	TEST( SourceRef, pos )
	{
		...
		WANT_EQ( "test:1", ref.textRefLine().stdstr() );
		WANT_EQ( "test:1:6", ref.textRefPos().stdstr() );
		WANT_EQ( "test:1:6-10", ref.textRefRange().stdstr() );
	}
}
