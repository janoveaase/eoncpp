...
namespace eon
{
	namespace source
	{
		...
		EON_TEST_3STEP_SANDBOX( File, File, name,
			...
			File obj( string( ( sandboxDir() / "source.txt" ).string() ) ),
		...
		EON_TEST_2STEP_SANDBOX( File, numBytesInSource, empty,
			saveFile( "source.txt", "" ),
			EON_EQ( 0, File( string( ( sandboxDir() / "source.txt" ).string() ) ).numBytesInSource() ) );
		EON_TEST_2STEP_SANDBOX( File, numBytesInSource, non_empty,
			saveFile( "source.txt", "one two three" ),
			EON_EQ( 13, File( string( ( sandboxDir() / "source.txt" ).string() ) ).numBytesInSource() ) );
		...
		EON_TEST_3STEP_SANDBOX( File, getPosAtOffset, zero,
			saveFile( "source.txt", "one two three" ),
			Pos base( 5, 5, 0, 5 ),
			EON_EQ( base, File( string( ( sandboxDir() / "source.txt" ).string() ) ).getPosAtOffset( base, 0 ) ) );
		EON_TEST_3STEP_SANDBOX( File, getPosAtOffset, positive_one,
			saveFile( "source.txt", "one two three" ),
			Pos base( 5, 5, 0, 5 ),
			EON_EQ(
				Pos( 6, 6, 0, 6 ), File( string( ( sandboxDir() / "source.txt" ).string() ) ).getPosAtOffset( base, 1 ) ) );
		EON_TEST_3STEP_SANDBOX( File, getPosAtOffset, negative_one,
			saveFile( "source.txt", "one two three" ),
			Pos base( 5, 5, 0, 5 ),
			EON_EQ(
				Pos( 4, 4, 0, 4 ),
				File( string( ( sandboxDir() / "source.txt" ).string() ) ).getPosAtOffset( base, -1 ) ) );
		EON_TEST_3STEP_SANDBOX( File, getPosAtOffset, positive_cross_line,
			saveFile( "source.txt", "one\ntwo\nthree" ),
			Pos base( 5, 5, 1, 1 ),
			EON_EQ(
				Pos( 9, 9, 2, 1 ).rawStr(),
				File( string( ( sandboxDir() / "source.txt" ).string() ) ).getPosAtOffset( base, 4 ).rawStr() ) );
		EON_TEST_3STEP_SANDBOX( File, getPosAtOffset, negative_cross_line,
			saveFile( "source.txt", "one\ntwo\nthree" ),
			Pos base( 5, 5, 1, 1 ),
			EON_EQ(
				Pos( 1, 1, 0, 1 ),
				File( string( ( sandboxDir() / "source.txt" ).string() ) ).getPosAtOffset( base, -4 ) ) );

		EON_TEST_3STEP_SANDBOX( File, match, str_positive,
			saveFile( "source.txt", "one two three" ),
			string value = "two",
			EON_TRUE(
				File( string( ( sandboxDir() / "source.txt" ).string() ) ).match(
					Pos( 4, 4, 0, 4 ), Pos( 7, 7, 0, 7 ), value ) ) );
		EON_TEST_3STEP_SANDBOX( File, match, str_negative,
			saveFile( "source.txt", "one two three" ),
			string value = "two",
			EON_FALSE(
				File( string( ( sandboxDir() / "source.txt" ).string() ) ).match(
					Pos( 5, 5, 0, 5 ), Pos( 8, 8, 0, 8 ), value ) ) );

		EON_TEST_2STEP_SANDBOX( File, match, cstr_positive,
			saveFile( "source.txt", "one two three" ),
			EON_TRUE(
				File( string( ( sandboxDir() / "source.txt" ).string() ) ).match(
					Pos( 4, 4, 0, 4 ), Pos( 7, 7, 0, 7 ), "two" ) ) );
		EON_TEST_2STEP_SANDBOX( File, match, cstr_negative,
			saveFile( "source.txt", "one two three" ),
			EON_FALSE(
				File( string( ( sandboxDir() / "source.txt" ).string() ) ).match(
					Pos( 5, 5, 0, 5 ), Pos( 8, 8, 0, 8 ), "two" ) ) );
		...
		EON_TEST_2STEP_SANDBOX( File, chr, first,
			...
			EON_EQ( '1', File( string( ( sandboxDir() / "source.txt" ).string() ) ).chr( Pos( 0, 0, 0, 0 ) ) ) );
		EON_TEST_2STEP_SANDBOX( File, chr, last,
			saveFile( "source.txt", "123456789" ),
			EON_EQ( '9', File( string( ( sandboxDir() / "source.txt" ).string() ) ).chr( Pos( 8, 8, 0, 8 ) ) ) );
		EON_TEST_2STEP_SANDBOX( File, chr, beyond_last,
			saveFile( "source.txt", "123456789" ),
			EON_EQ( nochar, File( string( ( sandboxDir() / "source.txt" ).string() ) ).chr( Pos( 9, 9, 0, 9 ) ) ) );
		...
		EON_TEST_2STEP_SANDBOX( File, byte, first,
			saveFile( "source.txt", "123456789" ),
			EON_EQ( '1', File( string( ( sandboxDir() / "source.txt" ).string() ) ).byte( 0 ) ) );
		EON_TEST_2STEP_SANDBOX( File, byte, last,
			saveFile( "source.txt", "123456789" ),
			EON_EQ( '9', File( string( ( sandboxDir() / "source.txt" ).string() ) ).byte( 8 ) ) );
		EON_TEST_2STEP_SANDBOX( File, byte, beyond_last,
			saveFile( "source.txt", "123456789" ),
			EON_EQ( -1, File( string( ( sandboxDir() / "source.txt" ).string() ) ).byte( 9 ) ) );
		...
		EON_TEST_2STEP_SANDBOX( File, str, empty,
			...
			EON_EQ(
				"",
				File( string( ( sandboxDir() / "source.txt" ).string() ) ).str( Pos( 0, 0, 0, 0 ), Pos( 9, 9, 0, 9 ) ) ) );
		EON_TEST_2STEP_SANDBOX( File, str, non_empty,
			saveFile( "source.txt", "one two three" ),
			EON_EQ(
				"two",
				File( string( ( sandboxDir() / "source.txt" ).string() ) ).str( Pos( 4, 4, 0, 4 ), Pos( 7, 7, 0, 7 ) ) ) );
		...
		EON_TEST_2STEP_SANDBOX( File, bytes, empty,
			...
			EON_EQ(
				"",
				File( string( ( sandboxDir() / "source.txt" ).string() ) ).bytes(
					Pos( 0, 0, 0, 0 ), Pos( 9, 9, 0, 9 ) ) ) );
		EON_TEST_2STEP_SANDBOX( File, bytes, non_empty,
			saveFile( "source.txt", "one two three" ),
			EON_EQ(
				"two",
				File( string( ( sandboxDir() / "source.txt" ).string() ) ).bytes(
					Pos( 4, 4, 0, 4 ), Pos( 7, 7, 0, 7 ) ) ) );
		...
		EON_TEST_4STEP_SANDBOX( File, _forward, legal,
			saveFile( "source.txt", "one" ),
			Pos pos( 0, 0, 0, 0 ),
			File( string( ( sandboxDir() / "source.txt" ).string() ) )._forward( pos, 1 ),
			EON_EQ( 1, pos.BytePos ) );
		EON_TEST_4STEP_SANDBOX( File, _forward, legal_past_newline,
			saveFile( "source.txt", "one\ntwo" ),
			Pos pos( 0, 0, 0, 0 ),
			File( string( ( sandboxDir() / "source.txt" ).string() ) )._forward( pos, 4 ),
			EON_EQ( Pos( 4, 4, 1, 0 ), pos ) );
		EON_TEST_4STEP_SANDBOX( File, _forward, beyond_last,
			saveFile( "source.txt", "one" ),
			Pos pos( 0, 0, 0, 0 ),
			File( string( ( sandboxDir() / "source.txt" ).string() ) )._forward( pos, 10 ),
			EON_EQ( 3, pos.BytePos ) );
		EON_TEST_3STEP_SANDBOX( File, _forward, end_of_source,
			saveFile( "source.txt", "one" ),
			Pos pos( 4, 4, 0, 4 ),
			EON_RAISE( File( string( ( sandboxDir() / "source.txt" ).string() ) )._forward( pos, 1 ), EndOfSource ) );
		...
		EON_TEST_4STEP_SANDBOX( File, _backward, legal,
			saveFile( "source.txt", "one" ),
			Pos pos( 3, 3, 0, 3 ),
			File( string( ( sandboxDir() / "source.txt" ).string() ) )._backward( pos, 1 ),
			EON_EQ( 2, pos.BytePos ) );
		EON_TEST_4STEP_SANDBOX( File, _backward, legal_past_newline,
			saveFile( "source.txt", "one\ntwo" ),
			Pos pos( 7, 7, 1, 7 ),
			File( string( ( sandboxDir() / "source.txt" ).string() ) )._backward( pos, 4 ),
			EON_EQ( Pos( 3, 3, 0, 3 ), pos ) );
		EON_TEST_4STEP_SANDBOX( File, _backward, beyond_first,
			saveFile( "source.txt", "one" ),
			Pos pos( 3, 3, 0, 3 ),
			File( string( ( sandboxDir() / "source.txt" ).string() ) )._backward( pos, 10 ),
			EON_EQ( 0, pos.BytePos ) );
		EON_TEST_3STEP_SANDBOX( File, _backward, end_of_source,
			saveFile( "source.txt", "one" ),
			Pos pos( 10, 10, 0, 10 ),
			EON_RAISE( File( string( ( sandboxDir() / "source.txt" ).string() ) )._backward( pos, 2 ), EndOfSource ) );
