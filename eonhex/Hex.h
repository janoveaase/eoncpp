...
namespace eon
{
	...
	**************************************************************************/
	...
	public:
		...
		struct digits {
			digits() = default;
			inline digits( byte_t first, byte_t second ) {
				this->first = first; this->second = second; }
			inline std::string stdstr() const {
				std::string s; s += first; s += second; return s; }
	...
	public:
		...
		inline hex& operator=( hex&& other ) noexcept {
	...
	public:
		...
		static inline bool isDigit( byte_t byte ) noexcept {
			return ( byte >= '0' && byte <= '9' ) || ( byte >= 'A'
				&& byte <= 'F' ) || ( byte >= 'a' && byte <= 'f' ); }
