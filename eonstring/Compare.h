...
namespace eon::strcmp
{
	...
	{
	public:
		inline explicit icase_chr( const eon::locale* custom_locale = nullptr ) {
			Loc = custom_locale != nullptr ? custom_locale : &eon::locale::get(); }
		inline int operator()( char_t a, char_t b ) const noexcept
		{
			char_t lw_a = Loc->toLower( static_cast<wchar_t>( a ) );
			char_t lw_b = Loc->toLower( static_cast<wchar_t>( b ) );
			if( lw_a < lw_b )
				return -1;
			else
				return lw_a == lw_b ? 0 : 1;
		}
	...
	{
	public:
		inline explicit icase_utf8( const eon::locale* custom_locale = nullptr ) {
			Loc = custom_locale != nullptr ? custom_locale : &eon::locale::get(); }
		int operator()(
			const string_iterator& a_start,
			const string_iterator& a_end,
