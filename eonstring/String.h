...
namespace eon
{
	...
	**************************************************************************/
	...
	public:
		...
		//* Throws [eon::InvalidUTF8] if not a valid UTF-8!
		inline string( const char* buffer, size_t size ) {
			*this = std::string( buffer, size ); }
		...
		inline string( std::initializer_list<char_t> codepoints ) {
			*this = codepoints; }

		//* Construct from vector of Unicode codepoints
		//* Throws eon::[eon::InvalidUTF8] if not a valid code points!
		inline string( const std::vector<char_t>& codepoints ) {
			*this = codepoints; }
	...
	public:
		...
		//* Throws [eon::InvalidUTF8] if not valid codepoint
		inline string& operator=( char_t cp ) { uint32_t bytes;
			auto num = iterator::unicodeToBytes( cp, bytes );
			return assign( (const char*)&bytes, num ); }

		//* Assign from a single signed character
		//* Throws [eon::InvalidUTF8] if not valid UTF-8
		inline string& operator=( char c ) {
			return *this = static_cast<char_t>( c ); }
	...
	public:
		...
		inline size_t indentationLevel( char_t indentation_char = TabChr )
			const noexcept { return substr().indentationLevel(); }


		//* Get string as codepoints in a vector
		inline std::vector<char_t> chars() const {
			return substr().chars(); }

		//* Get substring as codepoints in a vector
		inline std::vector<char_t> chars( const substring& sub ) const {
	...
	public:
		...
		inline substring findFirst( char_t to_find ) const noexcept {
			return substr().findFirst( to_find ); }

		//* Find first occurrence of a codepoint, but limit search to the
		//* specified 'sub' [eon::substring].
		inline substring findFirst( char_t to_find, const substring& sub )
		...
		inline substring findFirst( char to_find ) const noexcept { return
			substr().findFirst( static_cast<char_t>( to_find ) ); }
		...
		inline substring findFirst( char to_find, const substring& sub )
			const noexcept { sub.assertSameBuffer( Bytes.c_str() );
				return sub.lowToHigh().findFirst( static_cast<char_t>(
					to_find ) ); }
		...
		//* of 'this' enclosed by double quotes. Any back-slashed quote mark in
		//* 'this' will be ignored (not counted as quotation mark).
		inline substring findFirstNotDoubleQuoted( char_t cp )
			const noexcept { return substr().findFirstIgnoreSections(
				cp, DblQuoteChr ); }
		...
		//* of 'this' enclosed by single quotes. Any back-slashed quote mark in
		//* 'this' will be ignored (not counted as quotation mark).
		inline substring findFirstNotSingleQuoted( char_t cp )
			const noexcept { return substr().findFirstIgnoreSections(
				cp, SglQuoteChr ); }
		...
		//* See [eon::substring::findFirstIgnoreSections] for how to customize
		//* searches.
		inline substring findFirstNotBraced( char_t cp, char_t brace = '(' )
			const noexcept {
		...
		inline substring findLast( char_t to_find ) const noexcept {
			return substr().highToLow().findLast( to_find ); }

		//* Find last occurrence of a codepoint, but limit search to the
		//* specified 'sub' [eon::substring].
		inline substring findLast( char_t to_find, substring sub ) const
		...
		inline substring findLast( char to_find ) const noexcept {
			return substr().highToLow().findLast( static_cast<char_t>(
				to_find ) ); }
		...
		inline iterator findLastNotOf( const string& characters ) const
			noexcept { return substr().highToLow().findLastNotOf(
				characters.substr() ); }
	...
	public:
		...
		inline size_t count( char_t to_count ) const noexcept {
		...
		inline size_t count( char_t to_count, const substring& sub )
			const noexcept { sub.assertSameBuffer( Bytes.c_str() );
	...
	public:
		...
		//* Returns a new iterator for the same position that where the insert
		//* as the old will be invalid after insert.
		inline iterator insert( const iterator& pos, char_t cp ) {
			return insert( pos, string( cp ) ); }
	...
	public:
		...
		//* Get [eon::substring] before first occurrence of 'delimiter'
		inline substring beforeFirst( char_t delimiter ) const noexcept {
		...
		//* Get [eon::substring] before last occurrence of 'delimiter'
		inline substring beforeLast( char_t delimiter ) const noexcept {
		...
		//* Get [eon::substring] after first occurrence of 'delimiter'
		inline substring afterFirst( char_t delimiter ) const noexcept {
		...
		//* Get [eon::substring] after last occurrence of 'delimiter'
		inline substring afterLast( char_t delimiter ) const noexcept {
		...
		//* Get a copy of the entire string with all occurrences of 'find'
		//* within 'sub' [eon::substring] replaced with 'replacement'
		string replace( char_t find, char_t replacement, const substring& sub )
			const;

		//* Get a copy of the entire string with all occurrences of 'find'
		//* replaced with 'replacement'
		inline string replace( char find, char replacement ) const {
			return replace( static_cast<char_t>( find ),
				static_cast<char_t>( replacement ) ); }

		//* Get a copy of the entire string with all occurrences of 'find'
		//* within 'sub' [eon::substring] replaced with 'replacement'
		inline string replace( char find, char replacement,
			const substring& sub ) const { return replace(
				static_cast<char_t>( find ), static_cast<char_t>(
					replacement ), sub ); }
		...
		//* Get a copy of the entire string with 'sub' [eon::substring]
		// replaced with 'replacement'
		inline string replace( const substring& sub, char_t replacement )
		...
		//* Get a copy of the entire string with all occurrences of 'to_remove'
		//* removed
		inline string remove( char_t to_remove ) const { return
			replace( string( to_remove ), string::Empty, substr() ); }

		//* Get a copy of the entire string with all occurrences of 'to_remove'
		//* within 'sub' [eon::substring] removed
		inline string remove( char_t to_remove, const substring& sub )
		...
		inline substring trimTrailing() const {
			return substr().trimTrailing(); }
		...
		//* it that long
		inline string padLeft( size_t target_size, char_t fill = SpaceChr )
			const { return NumChars < target_size
				? string( target_size - NumChars, fill ) += *this : *this; }

		//* Get a copy of the string, if it is shorter than 'target_size', add
		//* enough copies of the the 'fill' codepoint on the right side to make
		//* it that long
		inline string padRight( size_t target_size, char_t fill = SpaceChr )
			const { return NumChars < target_size
		...
		//* make it that long
		//* (Centers the string)
		string padLeftAndRight( size_t target_size, char_t fill = SpaceChr )
			const;
		...
		string indentLines( size_t indentation_level,
			char_t indentation_char = TabChr ) const;
		...
		friend inline string operator+( const char* a, const string& b ) {
			return string( a ) += b; }
		friend inline string operator+( const string& a, char_t b ) {
			return string( a ) += b; }
		friend inline string operator+( char_t a, const string& b ) {
			return string( a ) += b; }
	...
	public:
		...
		inline bool isFloat( char_t decimal_separator = PointChr )
		...
		inline bool isFloat( const substring& sub,
			char_t decimal_separator = PointChr ) const {
		...
		//* Example: 0001.0000 -> 1
		inline substring trimNumber( char_t decimal_separator = PointChr )
			const { return substr().trimNumber( decimal_separator ); }
		...
		inline substring trimFloat( char_t decimal_separator = PointChr )
			const { return substr().trimFloat( decimal_separator ); }


		//* Get a copy of the number string with the integer part separated
		//* into thousands.
		//* Example: 34525525.4621 -> 34,525,525.4621
		string separateThousands( char_t thousands_sep = CommaChr,
			char_t decimal_separator = PointChr ) const;
	...
	public:
		...
		inline int compare( char_t cp ) const noexcept {
			return NumChars == 0 || *begin() < cp ? -1
				: *begin() > cp ? 1 : NumChars == 1 ? 0 : 1; }
		inline bool operator<( char_t cp ) const noexcept {
			return compare( cp ) < 0; }
		inline bool operator<=( char_t cp ) const noexcept {
			return compare( cp ) <= 0; }
		inline bool operator>( char_t cp ) const noexcept {
		...
		inline int compare( char c ) const noexcept {
			return compare( static_cast<char_t>( c ) ); }
		inline bool operator<( char c ) const noexcept {
			return compare( static_cast<char_t>( c ) ) < 0; }
		inline bool operator<=( char c ) const noexcept {
			return compare( static_cast<char_t>( c ) ) <= 0; }
		inline bool operator>( char c ) const noexcept {
			return compare( static_cast<char_t>( c ) ) > 0; }
	...
	public:
		...
		static inline bool isLetter( char_t cp ) noexcept { return
			isLetterUpperCase( cp ) || isLetterLowerCase( cp )
			|| isLetterTitleCase( cp ) || isLetterModifier( cp )
			|| isLetterOther( cp ); }
		static inline bool isLetterUpperCase( char_t cp ) noexcept {
			return Characters::get().isLetterUpperCase( cp ); }
		static inline bool isLetterLowerCase( char_t cp ) noexcept {
			return Characters::get().isLetterLowerCase( cp ); }
		static inline bool isLetterTitleCase( char_t cp ) noexcept {
			return Characters::get().isLetterTitleCase( cp ); }
		static inline bool isLetterModifier( char_t cp ) noexcept {
			return Characters::get().isLetterModifier( cp ); }
		static inline bool isLetterOther( char_t cp ) noexcept {
			return Characters::get().isLetterOther( cp ); }
		//*
		static inline bool isMarkSpacingCombining( char_t cp )
			noexcept { return Characters::get().isMarkSpacingCombining( cp ); }
		static inline bool isMarkNonSpacing( char_t cp ) noexcept {
			return Characters::get().isMarkNonSpacing( cp ); }
		...
		static inline bool isNumberDecimalDigit( char_t cp )
			noexcept { return Characters::get().isNumberDecimalDigit( cp ); }
		static inline bool isNumberLetter( char_t cp ) noexcept {
			return Characters::get().isNumberLetter( cp ); }
		static inline bool isNumberOther( char_t cp ) noexcept {
			return Characters::get().isNumberOther( cp ); }
		//*
		static inline bool isPunctuation( char_t cp ) noexcept {
			return Characters::get().isPunctuation( cp ); }
		static inline bool isPunctuationConnector( char_t cp )
			noexcept { return Characters::get().isPunctuationConnector( cp ); }
		static inline bool isPunctuationDash( char_t cp ) noexcept {
			return Characters::get().isPunctuationDash( cp ); }
		static inline bool isPunctuationOpen( char_t cp ) noexcept {
			return Characters::get().isPunctuationOpen( cp ); }
		static inline bool isPunctuationClose( char_t cp ) noexcept {
			return Characters::get().isPunctuationClose( cp ); }
		static inline bool isPunctuationInitialQuote( char_t cp )
			noexcept {
			return Characters::get().isPunctuationInitialQuote( cp ); }
		static inline bool isPunctuationFinalQuote( char_t cp )
			noexcept { return Characters::get().isPunctuationFinalQuote(
				cp ); }
		static inline bool isPunctuationOther( char_t cp ) noexcept {
			return Characters::get().isPunctuationOther( cp ); }
		//*
		static inline bool isSymbol( char_t cp ) noexcept {
			return Characters::get().isSymbol( cp ); }
		static inline bool isSymbolCurrency( char_t cp ) noexcept {
			return Characters::get().isSymbolCurrency( cp ); }
		static inline bool isSymbolModifier( char_t cp ) noexcept {
			return Characters::get().isSymbolModifier( cp ); }
		static inline bool isSymbolMath( char_t cp ) noexcept {
			return Characters::get().isSymbolMath( cp ); }
		static inline bool isSymbolOther( char_t cp ) noexcept {
			return Characters::get().isSymbolOther( cp ); }
		//*
		static inline bool isSeparator( char_t cp ) noexcept {
			return Characters::get().isSeparator( cp ); }
		static inline bool isSeparatorLine( char_t cp ) noexcept {
			return Characters::get().isSeparatorLine( cp ); }
		static inline bool isSeparatorParagraph( char_t cp )
			noexcept { return Characters::get().isSeparatorParagraph( cp ); }
		static inline bool isSeparatorSpace( char_t cp ) noexcept {
			return Characters::get().isSeparatorSpace( cp ); }


		//* Check if a codepoint is a word character
		//* Word characters are: 'letters', 'number decimal digits' (of any
		//* kind) and 'underscore'.
		static inline bool isWordChar( char_t cp ) noexcept { return
			isLetter( cp ) || isNumberDecimalDigit( cp ) || cp == '_'; }

		//* Check if a codepoint is a space character
		//* Space characters are: 'character tabulation', 'space' and
		//* 'separators'
		static inline bool isSpaceChar( char_t cp ) noexcept {
			return ( cp >= 0x09 && cp <= 0x0D ) || isSeparator( cp ); }
	...
	private:
		...
		static size_t _findDecimalSeparator( std::vector<char_t>& digits,
			char_t decimal_separator = PointChr ) noexcept;
		...
		// a point!
		static void _roundUp( std::vector<char_t>& digits, size_t i )
			noexcept;
