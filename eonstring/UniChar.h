...
namespace eon
{
	...
	**************************************************************************/
	...
	public:
		...
		inline bool isLetter( char_t c ) const noexcept { return
		...
		//* Check if a character belongs to the "number" categotry
		inline bool isNumberAsciiDigit( char_t c ) const noexcept {
			return c >= '0' && c <= '9'; }
		inline bool isNumberDecimalDigit( char_t c ) const noexcept {
			return c <= 0x39 ? c >= 0x30 && c <= 0x39
				: std::binary_search( number_decimal_digit->begin(),
					number_decimal_digit->end(), c ); }
		inline bool isNumberLetter( char_t c ) const noexcept {
		...
		inline bool isPunctuation( char_t c ) const noexcept { return
		...
		inline bool isSeparatorLine( char_t c ) const noexcept {
			return c == 0x2028; }
		inline bool isSeparatorParagraph( char_t c ) const noexcept {
			return c == 0x2029; }
		inline bool isSeparatorSpace( char_t c ) const noexcept {
	...
	private:
		...
		const std::vector<char_t> letter_title_case{
		...
		const std::vector<char_t> punctuation_connector{
			...
			0xFE4F, 0xFF3F };
		const std::vector<char_t> punctuation_dash{ 0x002D, 0x058A,
		...
		const std::vector<char_t> punctuation_final_quote{ 0x00BB, 0x2019,
	...
	static inline bool isDigit( char_t codepoint ) noexcept {
